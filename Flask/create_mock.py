...
from Flask_class.hsahlib_password import encryption
...
def login():
    ...
    while tag:
        ...
        if not username:
            res = {'msg': '用户不能为空', 'msg_code': 2001}
            break
        if not pwd:
            res = {'msg': '用户密码不能为空', 'msg_code': 2001}
            break
        else:
            pwd = encryption(pwd=pwd)  # 密码加密在验证
        if not my_db(sql):
